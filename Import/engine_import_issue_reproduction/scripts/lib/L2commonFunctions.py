...
# Accessing cfg file from the caller script
def config_info():
    frm = inspect.stack()[-2]
...
#######################################################################################################################
#####################################################################################################################
def get_serviceIdlist_from_config(cfg):
    """
    To get the services from config  
    :param yaml config as object:
    :return: list of servies
    """
    cdvrServiceslist = []
    try:
	for k,v in cfg["test_channels"].items():
            if k.startswith("Generic"): 
                cdvrServiceslist.append(str(cfg["test_channels"][k]["ServiceId"]))
    except:
        print "Failed to get services from configuration "
    print cdvrServiceslist 
    return cdvrServiceslist
...
########################################################################################################################################
def get_cache_endpoint(cfg,header,cache_type_list, printflg=False):
   try :
...
def get_recId_from_contentplaybackURI(playbackurilist):
    ...
    try:
        ...
        for playbackuri in playbackurilist:
            id = re.search('recId=(.+?)(&.*?)?$', playbackuri)
            if id:
                recId = id.group(1)
    ...
    try:
        ...
        if recid_list:
            ...
            for recId in recid_list:
                ...
                port_rm = cfg['rm']['port']
                url = protocol + "://" + rm_host +":"+ str(port_rm)+"/recordingInfo/" + recId + "?contentType=MPEG4&mode=playback&responseFormat=json"
...
def verify_FanOut_value(cfg, segmentIdList, isid, timeout, batchid=1,maxfanout=0,common_copy=True):
    ...
    try:
        ...
        # cosEnv = "rio/active"
        # cosEnv = cfg['cos']['env']
        cosEnv = cfg['cos']['active_storage_path']
