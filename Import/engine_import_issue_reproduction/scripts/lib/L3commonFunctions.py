...
def _household_exists(url):
    ...
    try:
        ...
        r = sendURL("get", url, header=headers, server_timeout=10)
        if r.status_code == 200:
            result = True
        else:
            result = False
...
def delete_household(protocol,port_upm,householdid,upm_host):
    ...
    if check_test_flag:
        print "Planner is marked YES"
        if delete_planner(cfg, householdid):
            print "Planner Deleted", householdid
            return True
        else:
            print "Unable to Delete Planner", householdid
            return False
    ...
    try:
        ...
        r = sendURL("delete", url, header=headers, server_timeout=15)
...
def setCdvrSubscriptionOffers(cfg, householdid):
        ...
        try:
            check_test_flag = test_flag(cfg, "no_upm")
            if check_test_flag:
                print "Set CDVR Subscription does not work with Planner"
                return None
            ...
            for host in hosts:
                ...
                for offerid in offers:
                    ...
                    r = sendURL("put", url, header=headers, payload_content=payload)
            ...
            print "Error in L3 setCdvrSubscriptionOffers function.\n", str(e)
            return False
...
def getCdvrSubscriptionOffers(cfg):
    ...
    try:
        ...
        print "Error in L3 getCdvrSubscriptionOffers function.\n", str(e)
        return None
...
def fetch_full_details_diskquota(protocol,upm_host,port_upm,householdid,timeout,printflg=False):
    ...
    if check_test_flag == True:
        diskquota = get_planner_diskquota(cfg, householdid, timeout=timeout)
        if diskquota:
            dq = interval2sec(diskquota)
            if dq ==0:
                print "DiskQuota of Planner %s if 0" %householdid
                return False
            class ret:
                content = str(dq)
            print "DiskQuota of the Planner %s is %s" %(householdid, ret.content)
            return ret

        else:
            print "Unable to Get the DiskQuota of Planner %s" % householdid
            return False
...
def fetch_diskspace_left(protocol,pps_host,port_pps,householdid,timeout,printflg=False):
    ...
    if "no_upm" in cfg['test-flags']:
        message = "Unable to get the Free diskspace of Planner"
        print message
        return False
...
def create_ahousehold(cfg,protocol,port_upm,prefix,region,cmdcRegion,adZone,marketingTarget,householdid,enabledServices,upm_host):
    ...
    check_test_flag = test_flag(cfg, "no_upm")
    if check_test_flag == True:
        if create_planner(cfg, householdid):
            print "Planner Created -", householdid
            return True
        else:
            print "Unable to create planner - ", householdid
            return False
    ...
    if _household_exists(url):
        ...
        r = sendURL("put", url, payload_content=payload, header=headers, server_timeout=10)
        if r.status_code != 201:
            print r.status_code
            print r.headers
            print r.content
...
##############################################################################################################################################
def _household_exists(url, plannerId=None):
    """
    ** DUPLICATE **
    ...
    check_test_flag = test_flag(cfg, "no_upm")
    if check_test_flag == True:
        if planner_exists(cfg, plannerId, timeout=10):
            return True
        else:
            return False
...
def modify_diskQuota(cfg,protocol,upm_host,port_upm,householdid,diskquota,diskquota_headers_hh,timeout,printflg=False):
    ...
    check_test_flag = test_flag(cfg, "no_upm")
    if check_test_flag == True:
        if update_planner(cfg, householdid, diskQuota=diskquota):
            print "Planner %s diskquota updated %s" % (householdid, diskquota)
            return "PASS"
        else:
            print "Unable to update diskquota for planner  - ", householdid
            return "FAIL"
...
def get_contentID_withoutPRM(protocol,rm_host,contentplayuri,timeout,pps_headers,printflg=False):
    ...
    try:
        ...
        cfg = config_info()
        protocol = cfg['rm']['protocol']
        port_rm = cfg['rm']['port']
        url = protocol + "://" + rm_host +":"+ str(port_rm)+"/recordingInfo/" + scheduleId + "?contentType=MPEG4&mode=playback&responseFormat=json"
...
def get_numberoftuners(protocol,upm_host,upm_port,upm_headers,householdid,timeout):
    ...
    try:
        ...
        check_test_flag = test_flag(cfg, "no_upm")
        if check_test_flag == True:
            tq = get_planner_tunerquota(cfg, householdid)
            if tq:
                return tq
            else:
                return False
...
def set_numberoftuners(protocol,upm_host,upm_port,upm_headers,householdid,timeout,value):
    ...
    try:
        ...
        check_test_flag = test_flag(cfg, "no_upm")
        if check_test_flag == True:
            if update_planner(cfg, householdid, numOfTuners=value):
                print "Planner %s number of tuner updated to %s" % (householdid, value)
                return "PASS"
            else:
                print "Unable to update number of tuner for planner  - ", householdid
                return "FAIL"
...
def cleanup_household(cfg,pps_port,protocol,pps_host,householdid,pps_headers,timeout):
    ...
    stdout_file = blockPrint() #Disabling the Prints in all called functions
    try:
        ...
        if event_state :
            ...
            event_state = json.loads(event_state.content)
            for val in event_state:
                if 'seriesId' in val and val['uri'] and val['recurrence']=="SERIES" and val['seriesId'] not in skipcheck:
                    print val['uri']
                    uri_delete = val['uri']
        ...
        if event_state3 :
            ...
            for val in event_state4:
                if val['uri']:
                    ...
                    pass
        #Set the default values for Diskquota received from the Config File
        household_disk_quota = fetch_full_details_diskquota(protocol,upm_host,upm_port,householdid,timeout,printflg=False)
        if household_disk_quota and not (str(default_disk_quota) == household_disk_quota.content):
            modify_diskQuota(cfg, protocol, upm_host, upm_port, householdid, str(default_disk_quota), upm_headers1, timeout, printflg=False)

        # Set the default values for Tunerquota received from the Config File
        household_tuner_quota = get_numberoftuners(protocol,upm_host,upm_port,upm_headers,householdid,timeout)
        if household_tuner_quota and not (household_tuner_quota == str(default_tuner_quota)):
            set_numberoftuners(protocol,upm_host,upm_port,upm_headers1,householdid,timeout,str(default_tuner_quota))

        enablePrint(filename=stdout_file) #Re-enabling the prints to print the below line and all successive prints
        # print "Household %s catalogs cleaned up successfully" %householdid
    except:
        enablePrint(filename=stdout_file) #Re-enabling the prints to print the exception and all successive prints
        PrintException()
    finally:
        enablePrint(filename=stdout_file) #Reenabling the prints to print the final line and all successive prints
...
def get_currentandfuture_contentIddict(json_responce,title,serviceparameterlist=[]):
    ...
    for cont in contents:
        if 'contents' in cont:
            ...
            print "'Contents' block missing in Grid Response"
...
def check_time_based_book(pps_port,protocol,pps_host,householdid,timeout,programstarttimeiniso,starttimeinepoch,printflg):
    """
    ...
    try:
        ...
        if check_test_flag == True:
            if create_planner(cfg, householdid):
                ...
                print "Unable to create planner"
                return False

        protocol = cfg['upm']['protocol']
        upm_port = cfg['upm']['port']

        url = protocol + "://" + upm_host + ":" + str(upm_port) + "/upm/households/" + householdid
        if _household_exists(url):
            print "Household already present: " + url
            del_rslt = delete_household(protocol, upm_port, householdid, upm_host)
            if del_rslt:
                hh_result = _household_create(url, hh_create_payload, hh_headers)
                if hh_result:
                    result = True
        else:
...
def _household_create(url, hh_create_payload, hh_headers):
    ...
    print "Create household via " + url
    try:
        #r = requests.put(url, data=hh_create_payload, headers=hh_headers, timeout=10)
        r = sendURL("put", url, payload_content=hh_create_payload, header=hh_headers, server_timeout=10)
        if r.status_code != 201:
            print r.status_code
            print r.headers
            print r.content
                ...
                if r.content == "[]":
                    ...
                    for temp in contentId_list:
                        for val in event_state:
                            try:
                                if val["scheduleInstance"] == temp:
                                    print "Content %s in in state %s on channel %s "%(val["scheduleInstance"],val['state'],val['channelId'])
...
def send_xmlFile_CI(xmlFile, cfg):
    ...
    with open(xmlFile, 'r') as fptr:
        ...
        r = sendURL("put", url, header=header, payload_content=contents, server_timeout=timeout)
        #print r.status_code
        #print r.headers
        #print r.content
        if r is not None:
...
def get_sm_api_token(cfg,timeout):
    ...
    try:
        ...
        r = sendURL("post", url, header=header, payload_content=contents, server_timeout=timeout)
...
def add_household_to_external_authz(cfg, householdid, timeout):
    """
    Add the household to the external authz
    :param cfg:
    :param householdid:
    :return:
    """

    authz_host = cfg['external_authz']['host']
    authz_port = cfg['external_authz']['port']

    url = "http://"+str(authz_host)+":"+str(authz_port)+"/authz/planner/"+str(householdid)
    r = sendURL("PUT", url, timeout)
    if r is not None:
        if r.status_code != 200:
            print r.status_code
            print r.content
            ...
            print "Household %s added to External Authz" %householdid
            return True
    else:
        print "Household %s not added to External authz" %householdid
        return False


def tms_notification_start(cfg, timeout):
    try:
        tms_host = cfg['tms']['host']
        tms_port = cfg['tms']['port']

        url = "http://"+str(tms_host)+":"+str(tms_port)+"/start"
        print "\nTMS Notification received started with URL :", url
        r = sendURL("GET", url, timeout)
        if r is not None:
            if r.status_code != 200:
                print r.status_code
                ...
                print "TMS Notification Receiver Started"
                return True
        else:
            print "TMS Notification Receiver Not Started"
            return False
    except Exception as e:
        print "Error in TMS Notification Start function\n",str(e)
        return False


def tms_notification_stop(cfg, timeout):
    try:
        tms_host = cfg['tms']['host']
        tms_port = cfg['tms']['port']

        url = "http://" + str(tms_host) + ":" + str(tms_port) + "/stop"
        print "\nTMS Notification received stopped with URL :", url
        r = sendURL("GET", url, timeout)
        if r is not None:
            if r.status_code != 200:
                print r.status_code
                ...
                print "TMS Notification Receiver Stopped"
                return True
        else:
            print "TMS Notification Receiver Not Stopped"
            return False
    except Exception as e:
        print "Error in TMS Notification Stop function\n",str(e)
        return False


def tms_notification_get(cfg, timeout):
    try:
        tms_host = cfg['tms']['host']
        tms_port = cfg['tms']['port']

        url = "http://" + str(tms_host) + ":" + str(tms_port) + "/get"

        print "\nGet data from TMS Notification receiver via :", url
        r = sendURL("GET", url, timeout)
        if r is not None:
            if r.status_code != 200:
                print r.status_code
                ...
                print "\nData got from TMS Notification Receiver"
                #print r.content
                return r.content
        else:
            print "TMS Notification Receiver Not Started"
        ...
        print "Error in tms notification get function\n", str(e)
        #PrintException(True)
        return False


def tms_extract_data(cfg, handle, timeout):
    try:
        data = tms_notification_get(cfg, timeout)
        if not data:
            return False

        print "Extracting data from TMS notification\n"#, data
        final_list = []
        lines = data.split('\n')
        #print "Lines from TMS:", lines
        for ll in lines:
            if handle in ll:
        ...
        print "Error in tms_extract_data function\n",str(e)
        return False


def tms_verify_state(cfg, handle, state="ADDED", timeout=10, type="XML", printflg=False, value=None, skip_last_char=False):
    """
    Get the data from the TMS notificaitons based on the notification handle supplied and check whether the required state is in the data
    :param cfg: config parameter
    :param handle: notification handle to fetch
    :param state: State / Item that needs to be verified
    :param timeout: timeout value
    :param type: XML / JSON
    :param printflg: print flag
    :param value: Value that needs to be checked in the data obtained
    :param skip_last_char: if mentioned True the last char in the notification handle is removed and proceed further
                            <used while working in Series>
    :return: True / False
    """
    try:
        if skip_last_char:
            print "Skip last char in handle."
            handle = handle[:-1]
            print "Handle :", handle
        final_list = tms_extract_data(cfg, handle, timeout)
        if not final_list:
            print "Entry not present in TMS data"
            return False

        gg = list(final_list)

        print "Data extracted from TMS notification\n"
        if printflg:
            for k in gg:
                print k
        ...
        for i in range(0, len(gg)):
            hh = gg[i].split(":", 1)[0].strip()
            data = gg[i].split(":", 1)[1].strip()
            da = ast.literal_eval(data)
            print "TMS Data : ", da
            if state in da.values():
                if value:
                    if str(da['value']) == str(value):
                        data_found = True
                    else:
                        print "Required notification found but the value mismatch : %s " %da['value']
                        return False
                else:
                    data_found = True
                if type not in hh:
                    print "Required notification received but the request is in %s format" %type
                    return False

        return data_found

    except Exception as e:
        print "Error in tms_verify_state function\n",str(e)
        return False


def get_content_handle(response, contentid):
    try:
        final_response = []
        event_state = json.loads(response.content)
        for val in event_state:
            if val["scheduleInstance"] == contentid:
                if val["state"] == "BOOKED" or val["state"] == "RECORDING":
                    print "\nNotification Handle for %s" % (contentid)
                    handle = val['notificationHandles'][0]
                    contentURI = val['uri']
                    final_response.append(handle)
                    final_response.append(contentURI)
...
def externalAuthz_switch(cfg, plannerId, choice="ADD",timeout=2, authz_host=None):
        try:
                protocol = cfg['protocol']
...
if __name__=="__main__":
    ...
    print tms_verify_state(cfg, "59635d779ba2033f23211455", "ADDED", 10)
