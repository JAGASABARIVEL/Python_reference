...
######################################################
# TestcaseId: TC9835
#TestcaseSteps: Big Red Button - SRT with invalid copy type
#STEP1: Ingest an event of 10 minutes with the posttime of 10 Minutes
#       to the CI Host on channel 1 & channel 2.
# CH1 |--EV1--|
# CH2 |--EV2--|
#STEP2: Set the SRT table with invalid copy type
#STEP3: Fetch the grid response to collect the ingested content.
#STEP4: Book EV1 from hh1, hh2 on CH1, EV2 from hh3, hh4 on CH2
#STEP5: Confirm that the households has the booked item in booked state.
#STEP6: Confirm that all households has the item in recording state.
...
def doit_wrapper(cfg, printflg=False):
    ...
    status = 3
    test_id = "TC9835"
    tims_dict = {test_id: ["US80593", message, status],}
    print "%s: Big Red Button - SRT with invalid copy type."% (test_id)
    ...
    try:
        ...
        srt_content = [[source_region, "*", dest_region, "commons"],]
        ...
        start_delay = 5
        timeslotinminutes = 2#cfg["test_channels"]["longProgramLength"] * 2
        ...
        print "### STEP1: Ingest an event of n minutes with the posttime of \
            n Minutes to the CI Host ######## \n \n"
        ci_host = cfg['ci']['host']
        ci_port = cfg['ci']['port']
        ...
        print "Wait time for the catalog ingest to get synced with "\
            "the CI Host in seconds: " + str(ingest_delay)
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            print "### STEP2: Setting the SRT table with invalid copy type to confirm "\
                "recording contents is PPS initiated copy type. ###### \n \n"
            ...
            print "### STEP3: Fetching the grid response to collect the "\
                "ingested content. ##### \n \n"
            gridservicelistresponse = fetch_gridRequest(catalogueId, \
                protocol, cmdc_host, cmdc_port, serviceIdlist, region, timeout, printflg)
            ...
            print "### STEP4: PPS event-1(on hh1, hh2), event-2(on hh3, hh4) booking "\
                "respectively from channel 1 & channel 2 is being done. ##### \n \n"
            for householdid in householdid_list[:2]:
                result = do_PPSbooking\
                         (pps_port, protocol, pps_host, householdid, pps_headers,\
                          payload_ev1, content_ev1, timeout, printflg)
                if result != "PASS":
                    message = "Testcase Failed:Unable to do pps "\
                        "booking of content ID "+str(content_ev1)\
            ...
            print "### STEP5: Confirming that the households has "\
                   "the booked item in booked state. ##### \n \n"
            ...
            for householdid in householdid_list[:2]:
                catalogresult, catalogresponse = verify_booking\
                    (pps_port, protocol, pps_host, householdid, \
                     content_ev1, timeout)
            ...
            print "### STEP6: Confirming that all households has "\
                   "the item in recording state. ##### \n \n"
            for householdid in householdid_list[:2]:
                ...
                if recordingcatalogresult != "PASS":
                    message = "Testcase Failed: Content ID {0} in household "\
                              "{1} is not in the RECORDING "\
                              "state".format(content_ev1, householdid)
                    assert False, message
            ...
            print "### STEP7: Confirming that all households, recording "\
            ...
            assert riodev_recording_hh1["CopyType"] == copy_type and \
                riodev_recording_hh2["CopyType"] == copy_type, "Testcase Failed: "\
                "Content %s are not in expected copy type %s in household 1 "\
                "& household 2."% (content_ev1, copy_type)
            ...
            assert riodev_recording_hh3["CopyType"] == copy_type and\
                riodev_recording_hh4["CopyType"] == copy_type, "Testcase Failed: "\
                "Content %s are not in expected copy type %s in household "\
                "3 & household 4."% (content_ev1, copy_type)
            ...
            for householdid in householdid_list:
                cleanup_household(cfg, pps_port, protocol,\
                    pps_host, householdid, pps_headers, \
                    timeout)
            configureSRT_instance = ConfigureSRT()
