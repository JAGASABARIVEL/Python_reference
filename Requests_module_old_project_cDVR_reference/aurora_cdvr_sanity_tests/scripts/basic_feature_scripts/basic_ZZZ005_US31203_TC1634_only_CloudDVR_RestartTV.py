...
##########################################################################
# TC1634 :Create a household authorized for Cloud DVR and RestartTV
...
# STEP3:Verify that it has only CDVR and Restart TV Services
...
def doit_wrapper(cfg, printflg=False):
    ...
    status = 3
    tims_dict = {
        "TC1634": ["US31203", message, status]
    }
    print "TC1634 :Create a household authorized for Cloud DVR and RestartTV"
    ...
    try:
        ...
        service_list = ["RESTART_ENABLER","CDVR","ALLOW-CDVR-TIME-BASED-RECORDING"]
        restart_service_list = json.dumps(service_list)
        restart_service_list1 = restart_service_list.replace(" ","")
        print "cdvr and restart service list: " +str(restart_service_list1)
        ...
        print message
        tims_dict = update_tims_data(tims_dict, 1, message, ["TC1634"])
        return tims_dict
    ...
    try:
        ...
        if householdservicelist:
            print "###STEP3:Verify that it has only CDVR and Restart TV #####\n\n"
            ...
            return tims_dict
        if list_compare:
            print "Created Household has only CDVR and Restart TV"
