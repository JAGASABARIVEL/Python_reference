...
def doit_wrapper(cfg,printflg=False):
    try :
        ...
        status = 3
        tims_dict = {
                 "TC1641":["US31203",message,status],
                 "TC1642":["US31203",message,status],
                }
        print "\nUS31203: Create a household"
        print "\nTC1641:De-authorize a household for Cloud DVR, ensure existing recordings can not be played out"
        print "\nTC1642:De-authorize a household for Cloud DVR, ensure existing recordings are maintained"
        # announce
        ...
        print message 
        tims_dict = update_tims_data(tims_dict,1,message,["TC1641","TC1642"])
        return tims_dict
    for (cmdc_host,pps_host,upm_host) in hosts_list :
     try:
        ...
        recordedcontentresult,recordedcontentresponse = verify_recorded_state(port_pps,protocol,pps_host,householdid,contentid_list[0],timeout)
        if recordedcontentresult == "PASS":
            message ="Recording is still present after deleting the cdvr services"
            print message
            tims_dict = update_tims_data(tims_dict,0,message,["TC1642"])
            jsonrecordedcontent = json.loads(recordedcontentresponse.content)
            for items in jsonrecordedcontent:
                recordedtitle = items['title']
                contentplayuri = items['contentPlayUri']
        ...
        if playbk_rslt == "FAIL":
            ...
            tims_dict = update_tims_data(tims_dict,0, message, ["TC1641"])
            return tims_dict
        else:
            set_HouseholdEnabledService(protocol, upm_host, port_upm, householdid, cDVR_service_list, upm_headers, timeout)
            message = playbk_msg + ", Testcase failed"
            print message
            tims_dict = update_tims_data(tims_dict,1, message, ["TC1641"])
            return tims_dict
