...
##########################################################################
# TC1635 : Create a household authorized for Cloud DVR and RestartTV and CatchupTV
# STEP1:Delete the household if it exists or create the Household if it does not exist with only Restart Service
# STEP2:Fetch the the household Enabled Services
# STEP3:Verify that it has only Cloud DVR and RestartTV and CatchupTV Services
...
def doit_wrapper(cfg, printflg=False):
    ...
    status = 3
    tims_dict = {
        "TC1635": ["US31203", message, status]
    }
    print "TC1635 :Create a household authorized for Cloud DVR and RestartTV and CatchupTV"
    ...
    try:
        ...
        service_list = ["RESTART_ENABLER","CATCHUP_ENABLER","CDVR","ALLOW-CDVR-TIME-BASED-RECORDING"]
        restart_service_list = json.dumps(service_list)
        restart_service_list1 = restart_service_list.replace(" ","")
        print "cdvr,restarttv and catchuptv service list: " + str(restart_service_list1)
        ...
        print message
        tims_dict = update_tims_data(tims_dict, 1, message, ["TC1635"])
        return tims_dict
    ...
    try:
        ...
        if householdservicelist:
            print "###STEP3:Verify that it has only Cloud DVR and RestartTV and CatchupTV #####\n\n"
            list_compare = list_comp(
                restart_service_list1, householdservicelist)
        else:
            ...
            return tims_dict
        if list_compare:
            print "The Created Household has only Cloud DVR and RestartTV and CatchupTV"
            print "###STEP4:Fetch the Diskquota  and Tunerquota of the household #######\n\n"
        ...
        if (tuners_in_hh == default_tuner_quota) and (disk_quota == default_disk_quota):
            ...
            delete_household(protocol, upm_port, householdid, upm_host)
            print message
            tims_dict = update_tims_data(tims_dict, 0, message, ["TC1635"])
            return tims_dict
        else:
