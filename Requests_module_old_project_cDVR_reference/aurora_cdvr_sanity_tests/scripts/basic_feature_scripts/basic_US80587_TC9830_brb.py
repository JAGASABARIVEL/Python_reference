...
#########################################################################
# TestcaseId: TC9830
#TestcaseSteps: Big Red Button - SRT is configured as Common Copy
#STEP1: Ingest an event of 10 minutes with the posttime of 10 Minutes to the CI Host on channel 1 & 2.
#STEP2: Set the SRT table in such a way to record the contents with common copy type.
#STEP3: Fetch the grid response to collect the ingested content.
#STEP4: Book Event 1 from HH1 and HH2 on Channel 1,  Event 2 from HH3 on Channel 2
...
#STEP7: Confirm that all households, recording its content with copy type as common.
#########################################################################
...
def doit_wrapper(cfg, printflg=False):
    ...
    print "%s: Big Red Button - SRT is configured as Common Copy."% (test_id)
    ...
    try:
        ...
        if len(channelfilter1) < threshold_channels:
            ...
            print message
            tims_dict = update_tims_data(tims_dict, 1, message, [test_id, test_id2])
            return tims_dict
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            print "### STEP2: Setting the SRT table in such a way to "\
            "record the contents with common copy type. ###### \n \n"
            ...
            cmdc_port, serviceIdlist, region, timeout, printflg)
            ...
            for householdid in householdid_list[:2]:
                recordingcatalogresult, recordingcatalogresponse = verify_recording_state(pps_port,\
                                             protocol, pps_host, householdid, content_ev1, timeout)
            ...
            playbackuri_list = get_content_playbackURI(pps_host, \
                      pps_port, protocol, \
                      [content_ev2], householdid_list[2:3], timeout)
            contentidlist = get_contentid_from_recid(cfg, playbackuri_list, timeout)
            vmr_response_1 = get_vmr_response(cfg, contentidlist[0], timeout)
