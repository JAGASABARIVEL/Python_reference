...
##########################################################################
# TC1633 :Create a household authorized for Cloud DVR and CatchupTV
...
# STEP3:Verify that it has only CDVR and Catchup TV Services
...
def doit_wrapper(cfg, printflg=False):
    ...
    status = 3
    tims_dict = {
        "TC1633": ["US31203", message, status]
    }
    print "US31203: Create a household"
    print "TC1633 :Create a household authorized for Cloud DVR and CatchupTV"
    try:
        # announce
        ...
        service_list = ["CATCHUP_ENABLER","CDVR","ALLOW-CDVR-TIME-BASED-RECORDING"]
        restart_service_list = json.dumps(service_list)
        restart_service_list1 = restart_service_list.replace(" ","")
        print "Catchup and CDVR List: " + str(restart_service_list1)
        ...
        print message
        tims_dict = update_tims_data(tims_dict, 1, message, ["TC1633"])
        return tims_dict
    ...
    try:
        ...
        if householdservicelist:
            print "###STEP3:Verify that it has only CDVR and Catchup TV Services #####\n\n"
            ...
            return tims_dict
        if list_compare:
            print "Created Household has only CDVR and Catchup TV Services"
        ...
        if (tuners_in_hh == default_tuner_quota)and(disk_quota == default_disk_quota):
            ...
            delete_household(protocol, upm_port, householdid, upm_host)
            print message
            tims_dict = update_tims_data(tims_dict, 0, message, ["TC1633"])
            return tims_dict
        else:
