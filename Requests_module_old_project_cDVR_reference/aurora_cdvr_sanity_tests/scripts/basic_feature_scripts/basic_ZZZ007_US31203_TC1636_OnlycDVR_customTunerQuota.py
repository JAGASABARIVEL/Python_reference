...
#######################################################################################################################################################################################
# TC1636 : Create a household authorized for Cloud DVR only with custom Recording Tuner Quota value
#STEP1: Delete the household if it exists or create the Household without any Household enabled services and "numOfTuners":"2"
...
#STEP4: Get the Tunerquota and Storage quota of the Household and verify that the Tuner quota is the value passed in Step1 and Disk quota is the default one coming from global value
...
def doit_wrapper(cfg, printflg=False):
    ...
    status = 3
    tims_dict = {
        "TC1636": ["US31203", message, status]
    }
    print "US31203: Create a household"
    print "TC1636 : Create a household authorized for Cloud DVR only with custom Recording Tuner Quota value"
    try:
        # announce
        ...
        }""" % (householdid, tuners_tobe_set,region, cmdcRegion, adZone, marketingTarget, deviceid)
    except:
        ...
        print message
        tims_dict = update_tims_data(tims_dict, 1, message, ["TC1636"])
        return tims_dict

    try:
        print "###STEP1: Delete the household if it exists or create the Household without any Household enabled services and 'numOfTuners':2 #################\n\n"
        ...
        if list_compare:
            ...
            print "###STEP4: Get the Tunerquota and Storage quota of the Household and verify that the Tuner quota is the value passed in Step1 and Disk quota is the default one coming from global value ####\n\n"
            ...
            print "Tuner value fetched from household created: " , tuners_in_hh
        else:
        ...
        if (tuners_in_hh == tuners_tobe_set) and (disk_quota == default_disk_quota):
            message = "Testcase Passed: Set Tunerquota,Default Diskquota and newly created household Tunerquota,Diskquota are same"
            delete_household(protocol, upm_port, householdid, upm_host)
            print message
            tims_dict = update_tims_data(tims_dict, 0, message, ["TC1636"])
            return tims_dict
        else:
