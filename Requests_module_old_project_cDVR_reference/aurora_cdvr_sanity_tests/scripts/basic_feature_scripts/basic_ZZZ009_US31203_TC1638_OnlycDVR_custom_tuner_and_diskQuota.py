...
###########################################################################################################################################################
# TC1638 : Create a household authorized for Cloud DVR only with custom Recording Tuner Quota and custom Storage Quota values
#STEP1: Delete the household if it exists or create the Household without any Household enabled services and ""diskQuota"":""54000"" and ""numOfTuners"":""2""
#STEP2: Get the ""CDVR"" related services from the config and store in a list
#STEP3: Enable only those services received from Step2 on the Household
#STEP4: Get the Enabled Services for the same household and compare the list with ""step2"" list
#STEP5: Get the Tunerquota and Storage quota of the Household and verify that the Disk quota and Tuner Quota values passed in Step1 
#STEP6: Delete the household and update the results irrespective of pass or fail
############################################################################################################################################################
...
def doit_wrapper(cfg, printflg=False):
    ...
    status = 3
    tims_dict = {
        "TC1638": ["US31203", message, status]
    }
    print "US31203: Create a household"
    print "TC1638 : Create a household authorized for Cloud DVR only with custom Recording Tuner Quota and custom Storage Quota values"
    try:
        # announce
        ...
        }""" % (householdid, diskquota_tobe_set, tuners_tobe_set, region, cmdcRegion, adZone, marketingTarget, deviceid)
    except:
        ...
        print message
        tims_dict = update_tims_data(tims_dict, 1, message, ["TC1638"])
        return tims_dict

    try:
        print "###STEP1: Delete the household if it exists or create the Household without any Household enabled services and ""diskQuota"":""54000"" and ""numOfTuners"":""2"" #########\n\n"
        create_hh = create_household(
            protocol, upm_port, householdid, upm_host, hh_headers, hh_create_payload)
        if create_hh:
            print "###STEP2: Get the ""CDVR"" related services from the config and store in a list ########################################################\n\n"
            print "cdvr service list:" +str(cDVR_service_list1)
            print "###STEP3: Enable only those services received from Step2 on the Household ######################################################################################\n\n"
            set_HouseholdEnabledServices_only(protocol,upm_host,upm_port,householdid,cDVR_service_list,upm_headers1,timeout)
            print "###STEP4: Get the Enabled Services for the same household and compare the list with ""step2"" list ###############################################\n\n"
        ...
        if list_compare:
            ...
            print "###STEP5: Get the Tunerquota and Storage quota of the Household and verify that the Disk quota and Tuner Quota values passed in Step1 ###################################\n\n"
            ...
            tims_dict = update_tims_data(tims_dict, 1, message, ["TC1638"])
            return tims_dict
        print "###STEP6: Delete the household and update the results irrespective of pass or fail #############################################################\n\n"
        if (tuners_in_hh == tuners_tobe_set) and (disk_quota == diskquota_tobe_set):
            message = "Testcase Passed: Set tunerquota,Set Diskquota and newly created household Tunerquota,Diskquota are same"
            delete_household(protocol, upm_port, householdid, upm_host)
            print message
            tims_dict = update_tims_data(tims_dict, 0, message, ["TC1638"])
            return tims_dict
        else:
            message = "Testcase Failed: Set Tunerquota,Set Diskquota and newly created household Tunerquota,Diskquota are same"
            delete_household(protocol, upm_port, householdid, upm_host)
            print message
            tims_dict = update_tims_data(tims_dict, 1, message, ["TC1638"])
            return tims_dict
