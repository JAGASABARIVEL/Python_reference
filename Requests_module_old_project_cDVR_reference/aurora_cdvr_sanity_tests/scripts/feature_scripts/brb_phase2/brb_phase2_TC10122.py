...
'''#########################################################################
TestcaseId: TC10122
Testcase: BRB - Dual VMR - Redirect recording before scheduling from VMR1 to VMR2 based on 
          multiple channels and single channel with different copy type
...
3. Modify the SRT in such a way to record the contents as below,
   RR1|CH1|RR2|Unique
   RR1|*|RR2|Common
4. Perform booking on all four households for the ingested contents.
   4.1. Perform the booking from channel 1 on hh 1 & hh2.
   4.2. Perform the booking from channel 2 on hh 3 & hh4.
5. Wait for the program to start its recording on all four households.
6. Confirm from VMR API response that recorded contents are as per the rule set in SRT.
...
def doit_wrapper(cfg, printflg=False):
    ...
    try:
        ...
        srt_content = [[source_region, str(testchannel1_uuid), dest_region, unique_copy],
