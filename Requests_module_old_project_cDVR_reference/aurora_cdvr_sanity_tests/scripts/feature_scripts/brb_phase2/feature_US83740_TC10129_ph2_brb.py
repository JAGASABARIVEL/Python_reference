...
#########################################################################
# TestcaseId: TC10129
# TestcaseSteps: BRB - Dual VMR - Redirect recording before scheduling from VMR1 to VMR2 for a single channel with common copy type
...
# STEP 3: Modify the SRT in such a way to record the contents from channel 1 with common copy type in RR2 recording region.
#
#          RR1|CH1|RR2|Common
#
...
def doit_wrapper(cfg, printflg=False):
    ...
    tims_dict = {test_id: ["US80587", message, status],
                 test_id1: ["US80592", message, status]}
    print "%s: BRB - Dual VMR - Redirect recording before scheduling from VMR1 to VMR2 for a single channel with common copy type." % (test_id)
    print "%s: Big Red Button - Change SRT when recording is ongoing."% (test_id1)
    ...
    try:
        ...
        if "no_brb" in cfg["test-flags"]:
            ...
            print message
            tims_dict = update_tims_data(tims_dict, 2, message, [test_id, test_id1])
            return tims_dict
        ...
        srt_content = [[source_region, str(testchannel1_uuid), dest_region, "Common"]]
        ...
        eventtitle1 = "TC10129" + str(random.randint(1, 499))
        ...
        print message
        tims_dict = update_tims_data(tims_dict, 1, message, [test_id, test_id1])
        return tims_dict
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            tims_dict = update_tims_data(tims_dict, 0, message, [test_id, test_id1])
