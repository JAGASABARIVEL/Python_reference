...
#########################################################################
# TestcaseId: TC10138
# Testcase: BRB - Dual VMR - Redirect the request from all VMR from all channels to
#           a specific VMR with Common copy type
# 1. Ingest an event on channel 1 & channel 2 with 5 minutes duration and 5 minues post time.
# 2. Ensure four household present in the system pointing to RR1 recording region.
# 3. Modify the SRT in such a way to record the contents from any region and from any channel with
...
#   6.2. Content from channel 1 recorded on hh3 & hh4 with common copy type in RR2 recording region.
...
def doit_wrapper(cfg, printflg=False):
    ...
    print "%s: BRB - Dual VMR - Redirect the request from all VMR from all channels to a " \
          "specific VMR with Common copy type." % (test_id)
    ...
    try:
        ...
        eventtitle1 = "TC10138" + str(random.randint(1, 499))
    ...
    for (cmdc_host, pps_host, upm_host) in hostlist:
        try:
            ...
            print "### STEP3: Modify the SRT in such a way to record the contents from any " \
                  "region and from any channel with common copy type in RR2 recording region. ###\n"
            ...
            print "### STEP 5. Wait for the program to start its recording on all " \
                  "four households.####\n"
            ...
            assert riodev_recording_hh1["CopyType"] == common_copy and \
                   ...
                   region_hh1 == dest_region and region_hh2 == dest_region, \
                "Testcase Failed: Content %s are not in expected copy " \
                "type %s in household 1 & household 2." % (content_ev1, copy_type)

            print "## STEP6.2. Content from channel 1 recorded on hh3 & hh4 with common copy " \
                  "type in RR2 recording region. ##"
            ...
            assert riodev_recording_hh3["CopyType"] == common_copy and \
                ...
                region_hh3 == dest_region and region_hh4 == dest_region, \
                "Testcase Failed: Content %s are not in expected copy " \
                "type %s in household 3." % (content_ev1, copy_type)
