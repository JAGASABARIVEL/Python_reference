...
'''#########################################################################
TestcaseId: TC10141
Testcase: BRB - Dual VMR - Redirect recording before scheduling from a VMR to another VMR for a 
          single channel in Common copy mode and playback the recording once done
1. Ingest an event on channel 1 & channel 2 with 5 minutes duration and 5 mintues post time.
2. Ensure four household present in the system pointing to RR1 recording region.
3. Modify the SRT in such a way to record the contents from channel 1 with common copy 
...
7. Wait for the content to move to recorded state and confirm playback happening on all households.
    ...
    for (cmdc_host, pps_host, upm_host) in hostlist:
        try:
            ...
            print "### STEP5. Wait for the program to start its recording on all four " \
                  "households.####\n"
            broadcast_starttime = get_timedifference(broadcast_starttime, printflg)
            ...
            for household in householdid_list[:2]:
                recorded_res, recorded_response = verify_recorded_state(pps_port, protocol,
                                                                        pps_host, household,
                                                                        [content_ev1],
                                                                        timeout)
                ...
                for items in recordedcatalogcontent:
                    if items['scheduleInstance'] == content_ev1:
                        if items['state'] == "RECORDED":
                            ...
                            playback_event1_dict[household] = [contentplayuri, recordedtitle]
                ...
                for items in recordedcatalogcontent:
                    if items['scheduleInstance'] == content_ev2:
                        if items['state'] == "RECORDED":
                            contentplayuri = items['contentPlayUri']
                            recordedtitle = items['title']
                            playback_event2_dict[household] = [contentplayuri, recordedtitle]
            ...
            for hhid, val in playback_event1_dict.items():
                ...
                playback_rslt, playback_msg = playback_recordedevent(cfg, abspath, test, pps_headers,
                                                                     ...
                                                                     rm_host, contentplayback_host_aws,
                                                                     contentplayback_port, proxy_host,
                                                                     proxy_port, contentplayback_url_aws,
                                                                     val[0], val[1], hhid, timeout,
