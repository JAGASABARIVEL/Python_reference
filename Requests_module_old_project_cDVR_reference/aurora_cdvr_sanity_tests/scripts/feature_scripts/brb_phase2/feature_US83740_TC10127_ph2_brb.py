...
#########################################################################
# TestcaseId: TC10127
# TestcaseSteps: BRB - Dual VMR - Redirect recording before scheduling from VMR1 to VMR2 for a single channel.
# STEP 1: Ingest an event on channel 1 & channel 2 with 5 minutes duration and 5 minutes post time.
#
#         0     5     10
#         |     |     |
#         |-----|-----|	
...
# STEP 3: Modify the SRT in such a way to record the contents from channel 1 with default copy type in RR2 recording region.
#
#          RR1|CH1|RR2|NoChange
#
...
#    STEP 6.1: Content from channel 1 recorded on hh1 & hh2 with default copy type in RR2 recording region.
#    STEP 6.2: Content from channel 2 recorded on hh3 & hh4 with default copy type in RR1 recording region.
...
def doit_wrapper(cfg, printflg=False):
    ...
    print "%s: BRB - Dual VMR - Redirect recording before scheduling from VMR1 to VMR2 for a single channel." % (test_id)
    ...
    try:
        ...
        print "v2pv vmr details : ", cfg["feature"]["brb"]["Recorderegions"]
        v2pcVMR_host = cfg["vmr"]["instances"]["v2pcVMR"]
        standaloneVMR_host = cfg["vmr"]["instances"]["standaloneVMR"]
        default_copy_type = unique_copy
        ingest_delay = 120

        srt_content = [[source_region, str(testchannel1_uuid), dest_region, "NoChange"]]
        ...
        eventtitle1 = "TC10127" + str(random.randint(1, 499))
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            assert riodev_recording_hh1["CopyType"] == default_copy_type and \
                   riodev_recording_hh2["CopyType"] == default_copy_type and \
                ...
                "type %s in household 1 & household 2." % (content_ev1, default_copy_type)

            switch_vmr_host(cfg, v2pcVMR_host)
            ...
            assert riodev_recording_hh3["CopyType"] == default_copy_type and \
                   ...
                   riodev_recording_hh3["A8UpdateURL"].split('/')[5] == source_region and \
                   riodev_recording_hh4["A8UpdateURL"].split('/')[5] == source_region, \
                "Testcase Failed: Content %s are not in expected copy " \
                "type %s in household 3." % (content_ev2, default_copy_type)
