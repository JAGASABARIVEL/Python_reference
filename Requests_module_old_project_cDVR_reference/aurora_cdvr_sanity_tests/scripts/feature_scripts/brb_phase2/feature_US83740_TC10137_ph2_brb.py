...
#########################################################################
# TestcaseId: TC10137
# TestcaseSteps: BRB - Dual VMR - Redirect the request from all VMR from all channels to a specific VMR with Unique copy type.
...
# STEP 3. Modify the SRT in such a way to record the contents from any region and from any channel with unique copy type in RR2 recording region.
...
#   STEP 6.2. Content from channel 2 recorded on hh3 & hh4 with unique copy type in RR2 recording region.
...
def doit_wrapper(cfg, printflg=False):
    ...
    print "%s: BRB - Dual VMR - Redirect the request from all VMR from all channels to a specific VMR with Unique copy type." % (test_id)
    ...
    try:
        ...
        srt_content = [["*", "*", dest_region, "Unique"]]
        ...
        eventtitle1 = "TC10137" + str(random.randint(1, 499))
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            assert riodev_recording_hh3["CopyType"] == unique_copy and \
                   riodev_recording_hh4["CopyType"] == unique_copy and \
                   riodev_recording_hh3["A8UpdateURL"].split('/')[5] == dest_region and \
                   riodev_recording_hh4["A8UpdateURL"].split('/')[5] == dest_region, \
                "Testcase Failed: Content %s are not in expected copy " \
                "type %s in household 3." % (content_ev1, unique_copy)
