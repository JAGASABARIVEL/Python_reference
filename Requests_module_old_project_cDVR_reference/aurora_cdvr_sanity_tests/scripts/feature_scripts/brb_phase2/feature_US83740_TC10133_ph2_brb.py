...
#########################################################################
# TestcaseId: TC10133
# TestcaseSteps: BRB - Dual VMR - Redirect the request from all VMR from a single channel to a specific VMR.
...
# STEP 3. Modify the SRT in such a way to record the contents from any region and from channel 1 with default copy type in RR2 recording region.
...
#    STEP 6.1. Content from channel 1 booked on hh1 & hh2 with default copy type in RR2 recording region.
#    STEP 6.2. Content from channel 2 booked on hh3 & hh4 with default copy type in RR2 recording region.
...
def doit_wrapper(cfg, printflg=False):
    ...
    print "%s: BRB - Dual VMR - Redirect the request from all VMR from a single channel to a specific VMR." % (test_id)
    ...
    try:
        ...
        srt_content = [["*", str(testchannel1_uuid), dest_region, "NoChange"]]
        ...
        eventtitle1 = "TC10133" + str(random.randint(1, 499))
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            assert riodev_recording_hh3["CopyType"] == default_copy_type and \
                ...
                "Testcase Failed: Content %s are not in expected copy " \
                "type %s in household 3." % (content_ev1, default_copy_type)
