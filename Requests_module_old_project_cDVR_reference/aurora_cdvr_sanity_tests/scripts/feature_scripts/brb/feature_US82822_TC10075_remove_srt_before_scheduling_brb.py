...
# STEP8.1: Remove SRT File before EV3 and EV4 gets scheduled
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            assert riodev_recording_hh1["CopyType"] == copy_type and (
                ...
                "Testcase Failed: Content %s are not in expected copytype %s"
            ...
            assert riodev_recording_hh3["CopyType"] == copy_type, (
                "Testcase Failed: Content %s are not in expected copytype %s"
                " in household 3."% (content_ev4, copy_type))
