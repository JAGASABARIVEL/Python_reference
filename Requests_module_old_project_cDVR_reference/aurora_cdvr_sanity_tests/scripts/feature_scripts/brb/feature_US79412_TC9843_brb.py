...
###################################################################################################
# TestcaseId: TC9843
# TestcaseSteps: Big Red Button - disable SRT
...
# STEP4: Do the PPS Booking of the ingested event in 2 HH, verify booking is successful
# STEP5: Disable the SRT before the start time
# STEP6: Wait till the start time and verify the recording and copy type is common
###################################################################################################
...
def doit_wrapper(cfg, printflg=False):
    ...
    try:
        ...
        eventtitle1 = "tc9843" + str(random.randint(1, 499))
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            print "### STEP4: Do the PPS Booking of the ingested event in 2 HH, verify booking is successful ###\n"
            payload_ev1 = """{
            ...
            for householdid in [householdid1, householdid2]:
            ...
            print "### STEP5: Disable the SRT before the start time ###\n"
            ...
            print "Disabled the SRT check"

            print "### STEP6: Wait till the start time and verify the recording state of the event and the copy type ### \n"
            ...
            for householdid in [householdid1, householdid2]:
                ...
                assert recording_res == "PASS", "Testcase Failed: Content ID {0} is not in the RECORDING state for HH {1}".format(content_ev1, householdid)
            ...
            assert riodev_recording_hh1["CopyType"] == common_copy \
                   and riodev_recording_hh2["CopyType"] == common_copy, "Testcase Failed: Contents are not in common copy in some households."

            print "Contents are getting recorded in common copy in all households."

            message = "TestCase Passed : Event Recorded with SRT disabled and the copy type is common"
            ...
            configureSRT_instance = ConfigureSRT()
            configureSRT_instance.modify_srt(cfg, [[reg_src, "*", reg_dest, res_copy_type]])
