...
#################################################################
# TestcaseId: TC9832
# TestcaseSteps: Big Red Button - Change SRT to common from unique
# STEP1: Ingest two events of 10 min with the posttime of 10 Min to the CI Host
#       on channel 1(event-1, event-3) & channel 2(event-2, event-4).
# STEP2: Set the SRT table to unique copy type.
# STEP3: Fetch the grid response to collect the ingested content.
# STEP4: Book UC EV1 from HH1, HH2 on CH-1, UC EV2 from HH3 on CH-2
# STEP5: Confirm that the households has the items in booked state.
# STEP6: Confirm that all households has the booked items in recording state.
# STEP7: Confirm that all households, recording its content with copy type as unique.
# STEP8: Set the SRT table to common copy type.
# STEP9: Book UC EV3 from HH1, HH2 on CH-1, UC EV3 from HH3 on CH-2
# STEP10: Confirm that the households has the items in booked state.
# STEP11: Confirm that all households has the booked item in recording state.
# STEP12: Verify 2 Common copy recordings (1 per program).
# Ingestion
# CH-1 |--EV1--|--EV3--|
# CH-2 |--EV2--|--EV4--|
#################################################################
...
def doit_wrapper(cfg, printflg=False):
    ...
    test_id2 = "TC9830"
    test_id3 = "TC9831"
    tims_dict = {test_id: ["US80590", message, status],
                 test_id1: ["US79250", message, status],
                 test_id2: ["US80587", message, status],
                 test_id3: ["US80589", message, status]}
    print "%s: Big Red Button - Change SRT to common from unique." % (test_id)
    print "%s Big Red Button - static_routing_table.txt file"\
           "edited before event SCHEDULE and event RECORD." % (test_id1)
    print "%s: Big Red Button - SRT is configured as Common Copy." % (test_id2)
    print "%s: Big Red Button - SRT is configured as Unique Copy." % (test_id3)
    ...
    try:
        ...
        if "no_brb" in cfg["test-flags"]:
            ...
            print message
            tims_dict = update_tims_data(tims_dict, 2, message, [test_id, test_id1, test_id2, test_id3])
            return tims_dict
        ...
        if len(channelfilter1) < threshold_channels:
            ...
            print message
            tims_dict = update_tims_data(tims_dict, 1, message, [test_id, test_id1, test_id2, test_id3])
            return tims_dict
        ...
        channel1.add_to_lineup(serviceId=testchannel1, timeSlotLengthMinutes=timeslotinminutes, timeSlotCount=2,
        ...
        channel2.add_to_lineup(serviceId=testchannel2, timeSlotLengthMinutes=timeslotinminutes, timeSlotCount=2,
                               programIDPrefix=eventtitle1)
    ...
    for (cmdc_host, pps_host) in hostlist:
        try:
            ...
            print "### STEP4: PPS event-1(on hh1, hh2), event-2(on hh3) booking respectively " \
                  "from channel 1 & channel 2 is being done. ##### \n \n"
            for householdid in householdid_list[:2]:
                ...
                if result != "PASS":
                    ...
                    print message
                    tims_dict = update_tims_data(tims_dict, 1, message, [test_id, test_id1, test_id3])
                    message = "Testcase Skipped: %s is skipped since %s, %s failed." % (test_id, test_id1, test_id2)
                    tims_dict = update_tims_data(tims_dict, 2, message, [test_id2])
            ...
            for householdid in householdid_list[2:3]:
                ...
                if result != "PASS":
                    ...
                    print message
                    tims_dict = update_tims_data(tims_dict, 1, message, [test_id, test_id1, test_id3])
                    message = "Testcase Skipped: %s is skipped since %s, %s failed." % (test_id2, test_id, test_id1)
                    tims_dict = update_tims_data(tims_dict, 2, message, [test_id2])
            ...
            for householdid in householdid_list[2:3]:
                catalogresult, catalogresponse = verify_booking(pps_port, \
                                                                protocol, pps_host, householdid, content_ev2, timeout)
                print "Booking response :", catalogresult
                if catalogresult == "PASS":
                    jsonresponse = json.loads(catalogresponse.content)
                    for items in jsonresponse:
            ...
            print "Waiting additional %s seconds to workaround the state\
                change issue." % (recording_delay)
            time.sleep(recording_delay)

            print "### STEP6: Confirming that all households has the booked " \
                  "items in recording state. ##### \n \n"
            for householdid in householdid_list[:2]:
                recordingcatalogresult, recordingcatalogresponse = verify_recording_state \
                    (pps_port, protocol, pps_host, \
                     householdid, content_ev1, timeout)
                if recordingcatalogresult != "PASS":
                    message = "Testcase Failed: Content ID {0} in household {1} " \
                              "is not in the RECORDING state".format(content_ev1, householdid)
                    assert False, message
            ...
            for householdid in householdid_list[2:3]:
                ...
                if recordingcatalogresult != "PASS":
                    message = "Testcase Failed: Content ID {0} in household " \
                              "{1} is not in the RECORDING state".format(content_ev1, householdid)
                    assert False, message
            ...
            playbackuri_list = get_content_playbackURI(pps_host, pps_port, protocol, [content_ev2],
                                                       householdid_list[2:3], timeout)
            contentidlist = get_contentid_from_recid(cfg, playbackuri_list, timeout)
            vmr_response_1 = get_vmr_response(cfg, contentidlist[0], timeout)
            riodev_recording_hh3 = json.loads(vmr_response_1.content)[0]
            assert riodev_recording_hh3["CopyType"] == copy_type, \
                "Testcase Failed: Content %s are not in expected copy type %s in " \
                "household 3." % (content_ev1, copy_type)
            message = "Testcase Passed: SRT per %s behaved as expected." % (test_id3)
            tims_dict = update_tims_data(tims_dict, 0, message, [test_id3])

            print "### STEP8: Setting the SRT table in such a way to record the contents " \
                  "with common copy type. ###### \n \n"
            configureSRT_instance = ConfigureSRT()
            configureSRT_instance.modify_srt(cfg, srt_content_change1)

            print "### STEP9: PPS event-3(on hh1, hh2), event-4(on hh3) booking respectively " \
                  "from channel 1 & channel 2 is being done. ##### \n \n"
            for householdid in householdid_list[:2]:
                result = do_PPSbooking(pps_port, protocol, pps_host, householdid, \
                ...
                if result != "PASS":
                    ...
                    print message
                    tims_dict = update_tims_data(tims_dict, 1, message, [test_id, test_id1, test_id2])

            for householdid in householdid_list[2:3]:
                result = do_PPSbooking(pps_port, protocol, pps_host, householdid, \
                                       pps_headers, payload_ev4, content_ev4, timeout, printflg)
                if result != "PASS":
                    message = "Testcase Failed:Unable to do pps booking of " \
                              "content ID " + str(content_ev4) + " in household " + \
                              str(householdid)
                    print message
                    tims_dict = update_tims_data(tims_dict, 1, message, [test_id, test_id1, test_id2])

            print "### STEP10: Confirming that the households has the " \
                  "items in booked state. ##### \n \n"
            time.sleep(fetch_bookingcatalog_delay)
            # Getting Catalog Response for the Booked Catalog
            for householdid in householdid_list[:2]:
                catalogresult, catalogresponse = verify_booking(pps_port, protocol, \
                                                                pps_host, householdid, content_ev3, timeout)
                print "Booking response :", catalogresult
                if catalogresult == "PASS":
                    jsonresponse = json.loads(catalogresponse.content)
                    for items in jsonresponse:
            ...
            for householdid in householdid_list[2:3]:
                catalogresult, catalogresponse = verify_booking(pps_port, \
                                                                protocol, pps_host, householdid, content_ev4, timeout)
                print "Booking response :", catalogresult
                if catalogresult == "PASS":
                    jsonresponse = json.loads(catalogresponse.content)
                    for items in jsonresponse:
            ...
            print "### STEP11: Confirming that all households has the booked " \
            ...
            for householdid in householdid_list[:2]:
                ...
                if recordingcatalogresult != "PASS":
                    message = "Testcase Failed: Content ID {0} in household {1} " \
                              "is not in the RECORDING state".format(content_ev3, householdid)
                    assert False, message

            for householdid in householdid_list[2:3]:
                recordingcatalogresult, recordingcatalogresponse = verify_recording_state \
                    (pps_port, protocol, pps_host, householdid, content_ev4, timeout)
                if recordingcatalogresult != "PASS":
                    message = "Testcase Failed: Content ID {0} in household {1}\
                        is not in the RECORDING state".format(content_ev4, householdid)
                    assert False, message

            print "### STEP12: Confirming that all households, recording its " \
                  "content with copy type as common. ##### \n \n"
            copy_type = common_copy
            # TODO: Verify SRT is being reflected in Riodev.

            playbackuri_list = get_content_playbackURI(pps_host, pps_port, protocol, \
                                                       [content_ev3], householdid_list[:2], timeout)
            ...
            assert riodev_recording_hh1["CopyType"] == copy_type and \
                ...
                "in household 1 & household 2." % (content_ev3, copy_type)

            print "\n" * 5

            playbackuri_list = get_content_playbackURI(pps_host, pps_port, \
                                                       protocol, [content_ev4], householdid_list[2:3], timeout)
            contentidlist = get_contentid_from_recid(cfg, playbackuri_list, timeout)
            vmr_response_1 = get_vmr_response(cfg, contentidlist[0], timeout)
            riodev_recording_hh3 = json.loads(vmr_response_1.content)[0]
            assert riodev_recording_hh3["CopyType"] == copy_type, \
            ...
            message = "TestCase Passed : SRT per %s is working as expected." % (test_id)
            tims_dict = update_tims_data(tims_dict, 0, message, [test_id])
            message = "TestCase Passed : SRT per %s is working as expected." % (test_id1)
            tims_dict = update_tims_data(tims_dict, 0, message, [test_id1])
            message = "TestCase Passed : SRT per %s is working as expected." % (test_id2)
            tims_dict = update_tims_data(tims_dict, 0, message, [test_id2])
