...
def doit(cfg, printflg=False):
    ...
    if "no_upm" in cfg['test-flags'] or "upm_proxy" in cfg['test-flags']:
        message = "Skipping Enabler services as there is no UPM"
        print message
        return 0
...
def setCdvrSubscriptionOffers(cfg):
    ...
    hosts = [cfg['upm']['host']]
    if cfg['feature']['allinstances'] == True and cfg['upm'].get('instances'):
