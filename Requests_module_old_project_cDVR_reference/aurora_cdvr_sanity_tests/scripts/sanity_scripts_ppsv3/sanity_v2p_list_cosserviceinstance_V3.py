...
def doit(cfg, printflg=False):
    ...
    try:
        ...
        if instance_list:
            ...
            TIMS_testlog.append(1)
            TIMS_testlog.append(
                'service instance are not successfully retrived')
            return TIMS_testlog
        ...
        for instance in instance_list:
            ...
            url = v2p_protocol + "://" + pam_Node + ":" + \
                str(port) + "/v2/serviceinstances/" + instance
            print "List   cos endpoints via " + url
            r = sendURL("get", url, timeout, headers)
            if r is not None:
                if (r.status_code != 200):
                    print "could not List  cos endpoints for pam node: " + pam_Node
                    print r.status_code
                    print r.headers
                    print r.content
                    ...
                    if result['properties']['state'] == "active":
                        pass
                    else:
                        print "\n" + "#" * 20 + " DEBUG STARTED " + "#" * 20 + "\n"
                        print "\nList  cos endpoints = " + r.content
                ...
                print "could not List  cos endpoints for v2p master node: "
                service_check_counter = service_check_counter + 1
        ...
        TIMS_testlog.append(1)
        TIMS_testlog.append(
            'List  cos endpoints was not successful ' + message)
        return TIMS_testlog
