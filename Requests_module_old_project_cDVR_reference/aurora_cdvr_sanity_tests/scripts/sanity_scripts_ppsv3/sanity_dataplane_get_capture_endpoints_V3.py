...
def doit(cfg, printflg=False):
    ...
    if recording_api == "v2p":
        source_name = get_cache_endpoint(cfg, headers, ["cisco-vmr"])
        if source_name:
            pass_counter = 0
                    ...
                    if (r.status_code != 200):
                        ...
                        if len(jsondata) == 0:
                            ...
                            try:
                                    ...
                                    for keyproperties, valueproperties in contents['properties'].iteritems(
                                        ...
                                        if 'address' in keyproperties:
                                            ...
                                            if pingIPAddress(
                                                    vipaddress, printflg):
                                                printLog(
                                                    'Captured Endpoint {0} is ACTIVE'.format(vipaddress), printflg)
                                                printLog(
                                                    "Pinging the VIP " +
                                                    vipaddress +
                                                    "for the service instance " +
                                ...
                                TIMS_testlog.append(msg)
                                return TIMS_testlog
        # Return based on the count of captured endpoint. Return 1 if there is
        # no Endpoint captured for the service instance
