    ...
    try:
        ...
        for index, v2pmasternode in enumerate(v2p_masterNode):
            ...
            url = v2p_protocol + "://" + v2pmasternode + ":" + \
                str(port) + "/sm/v2/type/mediaflowinstances?view=combined"
                ...
                if (r.status_code != 200):
                    ...
                    for value in responce:
                        if value['properties']['systems']['state'] == "enable":
                            ...
                            message = "State: " + \
                                value['properties']['systems']['state'] + " for id: " + value['id']
                            App_instance_list.append(message)
        ...
        if verify_check == len(v2p_masterNode):
            ...
            TIMS_testlog.append(
                'Get workflow app instances list ran succesfully')
            return TIMS_testlog
        else:
            print "\nTestcase Failed: could not get app instance list for all v2p masternodes"
            TIMS_testlog.append(1)
            TIMS_testlog.append(
                'Get workflow app instances list was not successful')
            return TIMS_testlog
