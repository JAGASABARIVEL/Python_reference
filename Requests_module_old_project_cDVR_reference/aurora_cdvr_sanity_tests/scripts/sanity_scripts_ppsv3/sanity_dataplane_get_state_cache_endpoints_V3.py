...
def doit(cfg, printflg=False):
    ...
    headers_v2p = {'Authorization': Authorization}
    if recording_api == "v2p":
        ...
        if source_name:
            ...
            for host in v2p_host:
                url = v2p_protocol + "://" + host + ":" + \
                    str(v2p_port) + "/sm/v2/type/appinstancestatuses"
                            ...
                            try:
                                for val in jsondata:
                                    if str(val["name"]) == end_point:
                                        if str(
                                                val["properties"]["state"]) == "inservice":
        ...
        if len(serviceinstances) == 0:
            ...
            printLog(
                "List of Service Instances running for the host:\n" +
                str(serviceinstances),
                printflg)
            for serviceinstance in serviceinstances:
                if recording_api == "msr":
                    ...
                    url = mos_host + "/v2/serviceinstances/" + \
                        serviceinstance + "/statecacheendpoints"
                    ...
                    if (r.status_code != 200):
                        ...
                        if len(jsondata) == 0:
                            ...
                            try:
                                    ...
                                    for keyproperties, valueproperties in contents['properties'].iteritems(
                                        ...
                                        if 'address' in keyproperties:
                                            ...
                                            printLog(
                                                ...
                                                serviceinstance,
                                                printflg)
                                            # To check the Endpoint Active
                                            # Status
                                            if pingIPAddress(
                                                    vipaddress, printflg):
                                                printLog(
                                                    'State Cache Endpoint {0} is ACTIVE'.format(vipaddress), printflg)
                                                printLog(
                                                    "Pinging the VIP " +
                                                    vipaddress +
                                                    " for the service instance " +
                                ...
                                TIMS_testlog.append(msg)
                                return TIMS_testlog
        # Return based on the count of State Cache endpoint. Return 1 if there
        # is no Endpoint Cached for that capture cluster
