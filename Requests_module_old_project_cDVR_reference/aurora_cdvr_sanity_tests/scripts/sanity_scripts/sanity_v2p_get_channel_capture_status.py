...
def doit(cfg,printflg=False):
    ...
    for index,v2pmasternode  in enumerate(v2p_masterNode):
        channel_list = get_v2p_contentID(v2p_protocol,v2pmasternode,port1,headers,timeout,printflg)
        if channel_list:
        ...
        workflow_name_list = get_workflowname_list(v2p_protocol,v2pmasternode,port1,headers,asset_type,timeout)
        if workflow_name_list is None:
            ...
            for workflowname in workflow_name_list:
                url = v2p_protocol + "://" + v2pmasternode + ":" + str(port2) + "/v1/assetworkflows/" + workflowname +"/assets" 
                print "Get Channel Capture status via " + url
                r = sendURL ("get",url,timeout,headers)
                if r is not None :
                    if ( r.status_code != 200):
                        message = "could not Get Channel Capture status for v2p master node: " + v2pmasternode + "for workflowname :" + workflowname
                        print message
                        ...
                        printLog("\nGet Channel Capture status = " + r.content,printflg)
                            ...
                            if value['contentId'] in channel_list:
                                ...
                                for status in Capture_Status:
                                    if status['state'] == 'CAPTURING' :
                                        ...
                                        message = "workflow name: " + workflowname + " is in CAPTURING state" + "for channel Id : " + value['contentId'] 
                                        message_list.append(message)
                                    else:
                                        print "\n" + "#"*20 + " DEBUG STARTED "+ "#"*20+ "\n"
                                        print "\nGet Channel Capture status = " + r.content
                                        print "\n" + "#"*20 + " DEBUG ENDED   "+ "#"*20+ "\n"
                                        message = "workflow name: " + workflowname + " is not in CAPTURING state" + "for channel Id : " + value['contentId']           
                                        message_list.append(message)   
