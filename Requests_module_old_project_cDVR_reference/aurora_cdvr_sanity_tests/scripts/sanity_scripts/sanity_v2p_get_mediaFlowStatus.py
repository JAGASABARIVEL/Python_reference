...
def doit(cfg,printflg=False):
    ...
    for index,v2pmasternode  in enumerate(v2p_masterNode):
        ...
        url = v2p_protocol + "://" + v2pmasternode + ":" + str(port) + "/sm/v2/type/mediaflowstatuses"
        ...
        if r is not None :
            if ( r.status_code != 200):
                ...
                printLog("\nGet Media workflow status = " + r.content,printflg)
                ...
                for value in responce:
                    if value['properties']['state'] == 'inservice' :
                        ...
                        message = "state for id: " + value['id'] + ": "  + value['properties']['state'] + " v2p masternode: "  + v2pmasternode
                        message_list.append(message)   
                if state_check == 0:
