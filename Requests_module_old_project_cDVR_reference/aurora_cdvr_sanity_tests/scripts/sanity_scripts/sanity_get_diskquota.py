...
def doit(cfg, printflg=False):
    ...
    for index, host in enumerate(hosts):
        ...
        url = protocol + "://" + host + ":" + str(port) + "/upm/households/" + householdid + "/diskQuota"
        print "Get Disk Quota via " + url
        ...
        if r is not None :
            if r.status_code != 200:
                ...
                if r.content is None :
                    print "\n" + "#"*20 + " DEBUG STARTED "+ "#"*20+ "\n"
                    print "Disk Quota = " + r.content + " seconds"
                    print "\n" + "#"*20 + " DEBUG ENDED   "+ "#"*20+ "\n"
                any_host_pass = any_host_pass + 1
                printLog("Disk Quota = " + r.content + " seconds",printflg)
    if any_host_pass :
        msg = 'Testcase passed :Get diskquota ran successfully.'
        print msg
        TIMS_testlog.append(0)
        TIMS_testlog.append(msg)
        return TIMS_testlog
    else:
        msg = 'Testcase failed :Get disk quota was not successful'
        print msg
        TIMS_testlog.append(1)
        TIMS_testlog.append(msg)
        return TIMS_testlog
