...
def doit(cfg,printflg=False):
    ...
    for index,v2pmasternode  in enumerate(v2p_masterNode):
        ...
        url = v2p_protocol + "://" + v2pmasternode + ":" + str(port) + "/sm/v2/type/platforminstancestatuses"
        ...
        if r is not None :
            if ( r.status_code != 200):
                ...
                state_check = 0
                printLog("\nGet Platform Instance status = " + r.content,printflg)
                ...
                for value in responce:
                    if value['properties']['state'] == 'inservice' or value['properties']['state'] == 'idle':
                        ...
                        message =  "state for id: "+ value['id'] + ": "  + value['properties']['state'] + "for v2p masternode: " + v2pmasternode
                        message_list.append(message) 
                if state_check == 0:
                    Platform_instance  = Platform_instance + 1
                    message_list.append("all the state values are 'inservice' or 'idle' for: " + v2pmasternode)
        else:
            message = "could not Get Platform Instance status for v2p master node: " + v2pmasternode
            message_list.append(message)
