...
##########################################################################
#   GET household Enabled Services
##########################################################################
def doit(cfg,printflg=False):
    ...
    for index, host in enumerate(hosts):
        ...
        url = protocol + "://" + host + ":" + str(port) + "/upm/households/" + householdid + "/enabledServices"
        r = sendURL ("get",url,timeout, headers)
        print "Get Household Enabled Services via ", url
        if r is not None :
            if ( r.status_code != 200):
                ...
                printLog('Enabled Services\n'+ json.dumps(json.loads(r.content),indent = 4, sort_keys=False),printflg)

    if any_host_pass :
         msg = 'Testcase passed : GET household Enabled Services ran successfully.'
         print msg
         ...
         msg = 'Testcase failed : GET household Enabled Services was not successful.'
         print msg
         TIMS_testlog.append(1)
         TIMS_testlog.append(msg)
         return TIMS_testlog
