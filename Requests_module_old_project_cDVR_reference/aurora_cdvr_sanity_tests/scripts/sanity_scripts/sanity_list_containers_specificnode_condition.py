...
##########################################################################
#   Get List of containers for specific node and check conditions
##########################################################################
def doit(cfg,printflg=False):
    ...
    US = 'Get List of containers for specific node and check conditions'
    TIMS_testlog = []
        ...
        if nodes_list:
            ...
            if r is not None :
                if ( r.status_code != 200):
                    ...
                    for values in responce['status']['conditions']:
                      print " type:" , values['type'],", status: " , values['status']
                      any_host_pass = any_host_pass + 1
        else:
            print "Error in getting node list"
    if  any_host_pass :
         msg = 'Testcase passed :Get List of containers for specific node and check conditions ran succesfully'
         print msg
         TIMS_testlog.append(0)
         TIMS_testlog.append(msg)
         return TIMS_testlog
    else:
         msg = 'Testcase failed :Get List of containers for specific node and check conditions was not successful.'
         print msg
         TIMS_testlog.append(1)
         TIMS_testlog.append(msg)
         return TIMS_testlog
