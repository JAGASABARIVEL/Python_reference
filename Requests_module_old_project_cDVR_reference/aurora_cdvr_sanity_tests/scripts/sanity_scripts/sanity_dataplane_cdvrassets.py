...
def doit(cfg,printflg=False):
    ...
    if recording_api == "v2p":
         ...
         if asset :
             url =  v2p_protocal + "://" + v2p_host[0] + ":" + str(v2p_port) + "/v1/assetworkflows/" + asset + "/assets"
            ...
            try:
                for val in status_val:
                    if  val["status"]["state"] == "COMPLETE" :
                        printLog("Asset capture is COMPLETE for contentId" + val["contentId"],printflg)
                        printLog("assetMgmtUrl  " + val["assetMgmtUrl"],printflg)
                    elif val["status"]["state"] == "PENDING":
                        printLog("Asset capture is scheduled to start in future" + val["contentId"],printflg)
                        printLog("assetMgmtUrl  " + val["assetMgmtUrl"],printflg)
                    elif val["status"]["state"] == "CAPTURING":
                        printLog("Asset capture state : " + val["status"]["state"]+ " for contentId" + val["contentId"],printflg)
                        printLog("assetMgmtUrl  " + val["assetMgmtUrl"],printflg)
                    elif val["status"]["state"] == "FAILED":
                        printLog("Asset capture or publish failed due to " + val["status"]["state"],printflg)
                        printLog( val["status"]["reason"],printflg)
                    elif val["status"]["state"] == "DELETE_COMPLETE":
                        printLog( "Asset delete completed for contentId" + val["contentId"],printflg)
                        printLog("assetMgmtUrl  " + val["assetMgmtUrl"],printflg)
                    elif val["status"]["state"] == "DELETE_FAILED":
                        printLog( "Asset delete Failure for contentId" + val["contentId"],printflg)
                        printLog("assetMgmtUrl  " + val["assetMgmtUrl"],printflg)
