...
def is_itemids_booked(cfg, planner_id, bkin_itms_lst, timeout):
    ...
    try:
        print "[INFO: ] items list ", bkin_itms_lst
        count = 0
        for item in bkin_itms_lst:
            ...
            if result:
                ...
                if res_dict.get("state") == "BOOKED":
                    count += 1
                else:
                    return False, None
            else:
                return False, None
        if count == len(bkin_itms_lst):
...
def find_bookings_by_start_time(cfg, channel, starttime, plannerid, timeout=2, pps_host=None):
    ...
    try:
        ...
        url = protocol + "://" + pps_host + ":" + str(pps_port) + "/pps/v3/core/bookings/findBookingByStartTime?" \
