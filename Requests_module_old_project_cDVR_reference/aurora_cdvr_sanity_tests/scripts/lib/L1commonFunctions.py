...
def send_vmr_url(method, url, server_timeout=None, header=None, payload_content=None, auth=None):
    """
    Make the VMR request to the API method.
    :param method:
    :param url:
    :param server_timeout:
    :param header:
    :param payload_content:
    :param auth: haves the credentials to connect with VMR
    :return:
    """
    ...
    try:
        if(method == "get"):
            res = requests.get(
                url, headers=header, timeout=server_timeout,
                verify=False, allow_redirects=True, auth=auth)
            return res
        else:
            print "[ERROR: ] None of the protocol methods matched... "
            return None
    except requests.exceptions.RequestException as error: 
        print error
        print "Problem accessing ...... " + url
...
def get_utc_time_after_n_minutes(start_time_delay, convert_data = False):
    start_time_delay = start_time_delay
    cmd = Popen(["date", "-u", "-d", "+%s min"% (start_time_delay), "+%Y-%m-%d %H:%M:%S"], stdout=PIPE, stderr=PIPE)
    op = cmd.communicate()[0]
    if convert_data:
        op = op.split()[0] + 'T' + op.split()[1]
        return op
    else:
        return op
